<template>
	<mcm-page-area>
		...
		<mcm-page-search>
			<!--<el-form :inline="true" :model="query" class="">
				...
				<el-form-item label="客户群体">
					<el-select v-model="query.customerProfileId" clearable placeholder="请选择">
					    <el-option
					      v-for="item in clientData"
					      :key="item.customerProfileId"
					      :label="item.profileName"
					      :value="item.customerProfileId">
					    </el-option>
					  </el-select>
				</el-form-item>
		...
		<el-dialog title="审核" :visible.sync="auditShow" width="50%" class="dialog-audit">
			...
			<el-row :gutter="20" class="item" style="margin-top: 10px;">
				<el-col :span="4" style="text-align: right;">客户群体：</el-col>
				<el-col :span="18">
					<el-select v-model="options.customerProfileId" clearable placeholder="请选择">
					    <el-option
...
<script>
	import purchaserFun from '@f/action/customer/purchaser'
	export default {
		data () {
			return {
				query: {
		...
		created () {
			...
			this.dataFun.getClientGroupList();
		},
		methods: {
			...
			adudit (data) {
				...
				this.options.buyerId = data.buyerId;
			...
			disabled (data) {
				...
				this.dataFun.disabledPurchaser(options);
			}
		}
	}
</script>
